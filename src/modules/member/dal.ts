...
export const memberCommands = {
  ...
  async getPermissions(memberId: number): Promise<MemberPermissions> {
    const row = await getDb().maybeOne<{ permissions: string }>('SELECT permissions FROM member_permissions WHERE member_id = $1', [memberId]);
